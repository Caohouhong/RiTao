...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == _mediaArray.count && _mediaArray.count >= self.photoMaxNum) {
        
        [LCProgressHUD showInfoMsg:[NSString stringWithFormat:@"最多只能选择%d张",self.photoMaxNum]];
        return;
    }
    ...
    if (indexPath.row == _mediaArray.count) {
        switch (_type) {
            ...
            {
                LCActionSheet *sheet =  [LCActionSheet sheetWithTitle:@"选择图片来源" buttonTitles:@[@"相册", @"相机"] redButtonIndex:-1 delegate:nil];
                [sheet show];
...
//            }
        ...
        [DCURLRouter pushViewController:browser animated:YES];
    }
}
...
- (void)openAlbum {
    ...
    TZImagePickerController *imagePickerVc = [[TZImagePickerController alloc] initWithMaxImagesCount:self.photoMaxNum - _mediaArray.count delegate:self];
    ///是否 在相册中显示拍照按钮
