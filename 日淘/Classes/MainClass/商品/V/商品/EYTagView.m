...
-(void)setSelected:(BOOL)selected{
    ...
    if (selected) {
        self.layer.borderColor=_selectBgColor.CGColor;
        self.layer.borderWidth=1;
        [self setBackgroundColor:_selectBgColor];
        [self setTitleColor:_selectTextColor forState:UIControlStateSelected];
    } else {
...
- (EYCheckBoxButton *)tagButtonWithTag:(NSString *)tag
{
    ...
    tagBtn.selectTextColor = _selectTextColor;
    tagBtn.selectBgColor = _selectBgColor;
    tagBtn.selected=YES;
...
{
    if ([self.delegate respondsToSelector:@selector(tagViewDidClickWithTagStr:)]) {
        [self.delegate tagViewDidClickWithTagStr:sender.titleLabel.text];
    }
}
#pragma mark action
- (void)removeAllArrays
{
    [self removeTags:[NSArray arrayWithArray:_tagStrings]];
}
